...
void test_add()
{
	int output = add(1000, 2000);//corrected adjustment for the add in the test suite functions to catch errors associated with "int" being defined as an 8 bit integer rather than a 16 bit integer
	TEST_ASSERT_EQUAL(3000, output);
}
...
void test_is_equal()
{
	TEST_ASSERT_TRUE(is_equal(1000, 1000));
	TEST_ASSERT_FALSE(is_equal(1000, 2000));
}
...
void test_posnum(){
	TEST_ASSERT_EQUAL_STRING("True", posnum(13));
}
